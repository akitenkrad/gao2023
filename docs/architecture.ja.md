...
| `LLMPerceptionMechanism` | `Decision` | 各エージェントが inbox + memory から上位 K を scratch へ選択．既定 **規則ベース** (影響スコア = 時間減衰 + 関連性 + 真正性; LLM 呼び出し 0 回・bit 決定論的)．`--llm-perception` 時は候補を LLM に列挙提示し関連順の番号列を使う (`perception_prompt` + `parse_perception_ranking`)．Contagion と同一クライアント・キャッシュを共有し，有効番号が無ければ規則ベースへフォールバック |
...
- **LT** (Linear Threshold) / **IC** (Independent Cascade) — 進行性の二値伝染．socsim-mechanisms の `ThresholdContagionMechanism` / `SiContagionMechanism` と同系統だが，S³ のフォロー辺規約 (LT はフォロー先を参照，IC は配送と同様にフォロワへ押し出す) をそのまま共有するため `DiSocialNetwork` 上に直接実装した．
- **Voter** / **DeGroot** — 非進行性の二値合意，および連続線形合意．

各々 `baseline_<model>.csv` (`t`, `active_frac`, `mean_opinion`, `cumulative_reached`) を書き，Python `s3-tools reproduce` が S³ と重ねて描画する．
...
- Kempe, D., Kleinberg, J., & Tardos, É. (2003). *Maximizing the Spread of Influence through a Social Network.* KDD. (LT / IC ベースライン)
- DeGroot, M. H. (1974). *Reaching a Consensus.* JASA 69(345). (DeGroot ベースライン)
- Clifford, P., & Sudbury, A. (1973). *A Model for Spatial Conflict.* Biometrika 60(3). (Voter ベースライン)
